...
colnames(autism.data)[1] <- ""

names(autism.data)[7] <- "Player.Money.Lost"
```
...
date.string <- paste0( "1 " , autism.data$Month.Year )

#Convert to POSIXlt, a date-time format
strptime( date.string , format = "%d %B %Y" ) -> autism.data$Date
```
...
```{r}
brisbane.only<-autism.data[autism.data$Local.Govt.Area=="BRISBANE",]
```
